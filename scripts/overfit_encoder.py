#!/usr/bin/env python3
"""
Temporary script to overfit the Hiera MAE encoder to predict the learned latent parameters
from a previously run overfit_decoder experiment.
Visualizes the output by feeding the encoder's predictions through the frozen decoder.
"""
...
def reconstruct_and_save(model, latent, original_image, epoch, save_path):
    model.eval()
    with torch.no_grad():
...
def main():
    parser = argparse.ArgumentParser(description="Overfit Hiera encoder to target latent parameters.")
    parser.add_argument("--config", type=str, help="Path to config.yaml")
    parser.add_argument("--checkpoint", type=str, help="Path to decoder checkpoint containing target latent parameters")
    ...
    output_dir = REPO_ROOT / "experiments" / "overfit_encoder"
    ...
    if not chk_path:
        ...
        chk_path = str(REPO_ROOT / "experiments" / "overfit_decoder" / "overfit_checkpoint.pt")
        
    if args.smoke_test:
        print("Smoke test: Skipping checkpoint load, generating dummy target latent.")
        ...
        target_latent = torch.randn(1, num_mask_units, *mask_unit_spatial_shape_final, encoder_dim_out, device=device)
    else:
    ...
    # Filter out only encoder parameters for optimization
    encoder_params = [p for name, p in model.named_parameters() if p.requires_grad]
    ...
    param_groups = [
        {
            "params": [p for p in encoder_params if p.dim() > 1],
    ...
    for epoch in range(args.epochs):
        ...
        if config.get("grad_clip", 1.0) > 0:
            ...
            torch.nn.utils.clip_grad_norm_(encoder_params, config.get("grad_clip", 1.0))
        ...
        if (epoch + 1) % args.save_every == 0 or epoch == 0:
            ...
            reconstruct_and_save(model, latent, image, epoch, recon_path)
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE Loss")
    plt.title("Encoder Overfitting Loss Curve")
    plt.grid(True)
    ...
    reconstruct_and_save(model, latent, image, args.epochs, final_recon_path)
    
    # Save trained checkpoint (encoder weights)
    checkpoint_path = output_dir / "overfit_encoder_checkpoint.pt"
    torch.save({
        "epoch": args.epochs,
        "model_state_dict": {k: v for k, v in model.state_dict().items() if not (k.startswith("decoder_") or k.startswith("mask_token"))},
