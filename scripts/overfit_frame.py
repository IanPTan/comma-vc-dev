...
from scratch on a single frame from the dataset.
...
def reconstruct_and_save(model, original_image, epoch, save_path):
    model.eval()
    with torch.no_grad():
        ...
        # Forward pass through encoder and decoder (no masking)
        all_patches_mask = torch.ones((B, NH * NW), dtype=torch.bool, device=original_image.device)
        latent, _ = model.forward_encoder(original_image, mask_ratio=0.0, mask=all_patches_mask)
...
def main():
    parser = argparse.ArgumentParser(description="Overfit Hiera encoder & decoder end-to-end to a single image.")
    ...
    output_dir = REPO_ROOT / "experiments" / "overfit_frame"
    ...
    # Filter out parameters for optimization (train both encoder and decoder)
    # Ignore weight decay for 1D biases/norms and pos_embed/mask_token
    decay_params = []
    nodecay_params = []
    ...
    print("Starting end-to-end overfitting training...")
    loss_history = []
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.save_every == 0 or epoch == 0:
            ...
            reconstruct_and_save(model, image, epoch, recon_path)
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE Loss")
    plt.title("End-to-End Overfitting Loss Curve")
    plt.grid(True)
    ...
    reconstruct_and_save(model, image, args.epochs, final_recon_path)
    
    # Save trained checkpoint (full model)
    checkpoint_path = output_dir / "overfit_frame_checkpoint.pt"
    torch.save({
        "epoch": args.epochs,
